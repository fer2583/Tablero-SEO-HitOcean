import{kpis}from'./common.js';import{bar,countBy,fmt,num,donuts,miniRows}from'../services/utils.js';
export const meta={title:'Dashboard Ejecutivo',subtitle:'Vista gerencial del estado de migración + métricas en vivo (GSC, GA4, Clarity).'};
export function render(s){const r=s.filtered;const e=s.external;const done=Object.values(s.checkState).filter(Boolean).length,total=Math.max(s.data.checklist.length,1),ready=Math.round(done/total*100),high=r.filter(x=>/alta|high/i.test(x.priority)).length,meta=r.filter(x=>/optim|falta|incompleto|revisar/i.test(x.metaStatus)).length,redir=r.filter(x=>/301|redirect|redire/i.test(x.action)||num(x.status)===301).length,qa=r.filter(x=>!/aprobado/i.test(x.qa)).length,clicks=r.reduce((a,x)=>a+num(x.clicks),0),imp=r.reduce((a,x)=>a+num(x.impressions),0);return`${kpis(r,s)}${renderLiveKPIs(e)}<div class="grid2"><article class="card readiness"><small>Readiness migración</small><span class="big">${ready}%</span><p>${done} de ${s.data.checklist.length} tareas aprobadas.</p><div class="progress"><span style="width:${ready}%"></span></div></article><article class="card"><div class="chartTitle">Resumen ejecutivo</div><p class="sub">Hay <strong>${fmt.format(r.length)} URLs</strong>, <strong>${high}</strong> de prioridad alta, <strong>${meta}</strong> con metadata pendiente, <strong>${redir}</strong> redirects y <strong>${qa}</strong> tareas de QA pendientes. Acumulan <strong>${fmt.format(clicks)}</strong> clics y <strong>${fmt.format(imp)}</strong> impresiones.</p></article></div><div class="grid2"><article class="card"><div class="chartTitle">Indicadores circulares</div>${donuts([{label:'Alta prioridad',value:high,total:r.length},{label:'Metadata pendiente',value:meta,total:r.length},{label:'Redirects',value:redir,total:r.length}])}</article><article class="card"><div class="chartTitle">Top acciones</div>${miniRows(countBy(r,'action'))}</article></div><div class="grid2"><article class="card"><div class="chartTitle">Acciones</div>${bar(countBy(r,'action'))}</article><article class="card"><div class="chartTitle">Prioridad</div>${bar(countBy(r,'priority'))}</article><article class="card"><div class="chartTitle">Metadata</div>${bar(countBy(r,'metaStatus'))}</article><article class="card"><div class="chartTitle">Issues</div>${bar(countBy(r,x=>(x.issues||'').split(/\||,|;/)[0]||'Sin issue'))}</article></div>`}

/**
 * Renderizar KPIs en vivo desde GSC + GA4 + Clarity
 */
function renderLiveKPIs(e) {
  if (!e.kpis || !e.kpis.gsc_clicks && e.kpis.gsc_clicks !== 0) {
    return `<div class="grid2"><article class="card"><div class="chartTitle">📡 Métricas en vivo</div><p class="muted">Conectando con backend de APIs... <button class="btn ghost small" onclick="this.closest('.card').innerHTML='<p class=muted>Hacé clic en 📡 Live en el header para cargar datos.</p>'">📡 Cargar</button></p></article></div>`;
  }
  ...
  return `<div class="grid4">
    ...
    <article class="kpi ${(k.alerts_critical || 0) > 0 ? 'red' : 'gray'}">
      ...
      <small>${k.alerts_critical || 0} críticas · ${k.alerts_high || 0} altas</small>
    </article>
  </div>
  <div class="card" style="margin-bottom:0;padding:6px 12px;">
    <small class="muted">📡 Última actualización: ${updated} · Fuentes: GSC + GA4 + Clarity</small>
  </div>`;
}
